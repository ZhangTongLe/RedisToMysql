				...
				if (!file.isDirectory()) { 
					...
					while(keylist.hasNext())
					{
						...
						if(keysplit.length>=7)
						{
							...
							pcnt=redisserver.scard(key);
							if(pcnt>0&&(tag.equals("soccomm")||tag.equals("video")||tag.equals("search")||tag.equals("newsinfo"))){
					...
					if(num>0)//有数据存在才考虑进行数据库录入
					{
						...
						conn=DriverManager.getConnection(url);
						stmt =conn.createStatement();
						sql="delete from tb_mofang_hotspot_flow_today_tag where day='"+day+"' and tag<>\"新生\" and tag<>\"老生\"";
						stmt.execute(sql);
